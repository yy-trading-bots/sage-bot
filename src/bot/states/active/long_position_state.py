...
class LongPositionState(ActivePositionState):
    """
    Represents an active LONG trading position.
    ...
    def apply(self) -> None:
        """
        Apply the logic for managing an active LONG position.
        ...
        if self._is_tp_price():
            self._close_position("LONG", self._handle_tp)

        elif self._is_sl_price():
            self._close_position("LONG", self._handle_sl)
    ...
    def _is_tp_price(self) -> bool:
        ...
        Returns:
            bool: True if the current price is greater than the TP price; otherwise False.
        """
    ...
    def _is_sl_price(self) -> bool:
        ...
        Returns:
            bool: True if the current price is lower than the SL price; otherwise False.
        """
        snapshot = self.parent.data_manager.market_snapshot
